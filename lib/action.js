...
var action = function(spec, my) {
  ...
  if(spec.type === 'MUT' || spec.type === 'GET')
    my.type = spec.type || 'NONE';
  else
    my.type = 'NOP';
    
  if((/^[a-zA-Z0-9]+$/).exec(spec.subject))
 ...
 */
action.subject = { match: /^(MUT|GET):([a-zA-Z0-9]*)$/,
...
action.decapsulate = function(spec) {
  var subj = action.subject.match.exec(spec.msg.subject());
  if(subj) {
    spec.type = subj[1];
    spec.subject = subj[2];
...
action.encapsulate = function(a, msgtype) {
  ...
  msg.setSubject(action.subject.build(a));
