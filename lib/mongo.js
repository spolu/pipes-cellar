...
var mongo = function(spec, my) {
  ...
  my.ctx.log.debug('MONGO OPEN DB: ' + my.host + ':' + my.port + '/' + my.dbname);
  ...
  get = function(ctx, target, cb_) {
    var targ = targetfactory.match(target);
    if(targ) {
...
var collection = function(spec, my) {
  ...
  check = function() {    
    var expired = [];
    my.objects.forEach(function(obj, id) {
			 if(((new Date).getTime() - obj._lastWrite) > my.timeout) {
			   expired.push(id);
			 }
		       });    
    //my.ctx.log.debug('CHECK: expired len: ' + expired.length);
    for(var i = 0; i < expired.length; i ++) {
      delete my.objects[expired[i]];
    }
  };
	  ...
	  function(err, cursor) {
		...
		if(objs.length > 1) {
		  ...
		  my.objects[id] = {_lastWrite: new Date,
				    _cid: id};
                  my.objects[id]._hash = my.objects[id].makehash('_cid');
		  unlock();
		}
	      });
	  });	
  ...
  get = function(ctx, id, cb_) {
	...
	if(my.objects[id]) {
	  ...
	  delete res._id;
	  ctx.log.debug('GET ' + target(id) + ': ' + my.objects[id]._hash);
	  unlock(); cb_(res);
	}
      });
  };
  ...
  set = function(ctx, id, prevhash, obj, cb_) {
    if(id && obj && typeof obj === 'object') {      
      obj._cid = id; obj._hash = obj.makehash('_cid');
	...
	function(unlock) {
	  if(!my.objects[id]) {
		...
		if(err) { unlock(); ctx.error(err); return; }			  
		ctx.log.debug('SUCCESS ' + target(id) + ': ' + my.objects[id]._hash);
